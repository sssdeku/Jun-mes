$(function() {
	...
	function loadProductList(urlnew) {
		...
		pageNo = $("#productInPage .pageNo").val() || 1;
		keyword = $("#keyword").val();
		...
		if (urlnew != null) {
			...
			url = "/product/productInList.json";
		}
	...
	function renderProductListAndPage(result, url) {
		if (result.ret) {
			...
			if (result.data.total > 0) {
				...
				$('#productInList').html(rendered);
			} else {
				$('#productInList').html('');
			}
			bindProductClick();//更新操作
			var pageSize = $("#pageSize").val();
			var pageNo = $("#productInPage .pageNo").val() || 1;
			// 渲染页码
