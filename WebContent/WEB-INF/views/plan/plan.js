...
//			console.log($(dates[0]).val());
			...
			//each遍历checks（被选中的计划）
			//i表示checks集合下标，从0开始
			//check表示checks中的元素，相当于for循环中的this
			//closest方法返回第一个祖先tr标签，相当于返回被选中的那一行计划
			//attr方法为这个计划的tr标签添加属性data-id
			$.each(checks,function(i,check){
				var id=$(check).closest("tr").attr("data-id");
				ids+=id+"&";
			});
			//拼装ids
			ids=ids.substr(0,ids.length-1);//substr函数截取子字符串（表示从下标0开始，返回ids.length-1长度）
