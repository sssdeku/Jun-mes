...
<script id="paginateTemplates" type="x-tmpl-mustache">
<div class="col-xs-6">
    ...
    <div class="dataTables_paginate paging_simple_numbers" id="dynamic-table_paginate">
        <ul class="pagination">
            <li class="paginate_button previous {{^firstUrl}}disabled{{/firstUrl}}" aria-controls="dynamic-table" tabindex="0">
                <a href="#" data-target="1" data-url="{{firstUrl}}" class="page-actions">首页</a>
            </li>
            <li class="paginate_button {{^beforeUrl}}disabled{{/beforeUrl}}" aria-controls="dynamic-table" tabindex="0">
                <a href="#" name="前一页" data-target="{{beforePageNo}}" data-url="{{beforeUrl}}" class="page-actions">前一页</a>
            </li>
            ...
            <li class="paginate_button {{^nextUrl}}disabled{{/nextUrl}}" aria-controls="dynamic-table" tabindex="0">
                <a href="#" name="后一页" data-target="{{nextPageNo}}" data-url="{{nextUrl}}" class="page-actions">后一页</a>
            </li>
            <li class="paginate_button next {{^lastUrl}}disabled{{/lastUrl}}" aria-controls="dynamic-table" tabindex="0">
                <a href="#" data-target="{{maxPageNo}}" data-url="{{lastUrl}}" class="page-actions">尾页</a>
            </li>
            <li class="paginate_button {{^skipUrl}}disabled{{/skipUrl}}" aria-controls="dynamic-table" tabindex="0">
                <input  class="skiptxts" style="width:50px;float:left;background:lightgray;border:1px solid orange;" type="number" value="{{skipNo}}"/>
				<a href="#" data-target="{{pageNo}}" data-url="{{skipUrl}}" class="page-actions skip">跳转</a>
            </li>
        </ul>
    </div>
</div>
...
<script type="text/javascript">
	...
	var paginateTemplates = $("#paginateTemplates").html();
	//02将页面变量内容交给mustache库,使用mustache技术加载指定模板
	Mustache.parse(paginateTemplates);
	//渲染分页的内容
	//url total pageNo pageSize currentSize idElement:将paginatetemplate加载到哪一个页面版块 预留一个回调函数
	function renderPages(url, total, pageNo, pageSize, currentSize, idElement,
			callback) {
		//得到最大的页码,使用向上取整的函数
		...
		$("#" + idElement).html(Mustache.render(paginateTemplates, view));
		//每一个页码按钮拥有一个点击事件
		$(".page-actions").click(function(e) {
					//阻止默认事件，冒泡
					e.preventDefault();
					//获取当前的skipno
					var skipNo = $(".skiptxts").val();//1
